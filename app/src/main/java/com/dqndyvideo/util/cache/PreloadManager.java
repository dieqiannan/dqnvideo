...
private final static String TAG = "PreloadManager";
     ...
     * @param rawUrl 原始视频地址
     */
    public void addPreloadTask(TiktokBean bean,String rawUrl, int position) {
    ...
    private boolean isPreloaded(String rawUrl) {
        ...
        if(rawUrl.startsWith("http")){
            ...
            //先判断是否有缓存文件，如果已经存在缓存文件，并且其大小大于1KB，则表示已经预加载完成了
            File cacheFile = mHttpProxyCacheServer.getCacheFile(rawUrl);
            Log.e(TAG,"判断预加载 cacheFile="+cacheFile);
            if (cacheFile.exists()) {
                Log.e(TAG,"有本地缓存");
                //return true;

               if (cacheFile.length() >= 1024) {
                    Log.e(TAG,"判断预加载 >= 1024");
                    return true;
                } else {
                    Log.e(TAG,"加载 缓存太小");
                    //这种情况一般是缓存出错，把缓存删掉，重新缓存
